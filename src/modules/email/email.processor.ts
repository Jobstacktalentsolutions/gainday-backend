...
export class EmailProcessor extends WorkerHost {
  ...
  async process(job: Job<EmailJob | { recipients: string[]; subject: string; template: string; context?: Record<string, any> }>): Promise<any> {
    try {
      ...
      if ('recipients' in job.data) {
        ...
        const { template, subject, to, context, replyTo, cc, bcc } = job.data as EmailJob;
