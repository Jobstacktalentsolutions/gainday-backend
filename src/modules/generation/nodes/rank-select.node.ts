...
import { TaskCandidateJudgeScore, TaskCandidateRecord } from '../../../db/schema/job-extractions.schema';
...
export function rankSelectNode(ctx: GenerationContext) {
  return async (state: GenerationState): Promise<GenerationStateUpdate> => {
    const model = ctx.criticModel.withStructuredOutput(judgeScoreSchema);
    ...
    const scoreByCandidateId = new Map(result.scores.map((s) => [s.candidateId, s]));

    const scoredPool: TaskCandidateRecord[] = state.candidatePool.map((candidate) => {
    ...
    const ranked = [...scoredPool].sort(
      (a, b) => (b.judgeScore?.composite ?? -Infinity) - (a.judgeScore?.composite ?? -Infinity),
    );

    const selectedIds = new Set(ranked.slice(0, ctx.config.selectedTaskCount).map((c) => c.candidateId));

    const candidatePool = scoredPool.map((c) => ({ ...c, selected: selectedIds.has(c.candidateId) }));
