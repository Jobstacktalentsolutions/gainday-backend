...
import { categoryExtractionSchema, intentProblemExtractionSchema } from '../schemas/extraction.schema';
...
export function extractionNode(ctx: GenerationContext) {
  return async (state: GenerationState): Promise<GenerationStateUpdate> => {
    const categoryModel = ctx.generationModel.withStructuredOutput(categoryExtractionSchema);
...
stated, return null for problem. Never invent or infer a problem that is not present.`;
    ...
    const intentProblemModel = ctx.generationModel.withStructuredOutput(intentProblemExtractionSchema);
